...
from package.scripts.trino_worker import Worker
...
class TestWorker(unittest.TestCase):
    ...
    @patch('package.scripts.trino_worker.Worker.configure')
    @patch('package.scripts.trino_worker.Execute')
    def test_lifecycle_methods_shell_out_to_execute(
            self, execute_mock, unused_configure_mock):
        trino_worker = Worker()

        trino_worker.install(self.mock_env)
    ...
    @patch('package.scripts.trino_worker.create_connectors')
    def test_configure_adds_tpch_connector(self, create_connectors_mock):
        trino_worker = Worker()

        with patch('__builtin__.open'):
            trino_worker.configure(self.mock_env)
