...
from package.scripts.trino_cli import Cli
...
class TestCli(TestCase):
    ...
    @patch('package.scripts.trino_cli.Execute')
    def test_install_shells_out_to_execute(self, execute_mock):
        self.trino_cli.install(MagicMock)
    ...
    def test_status_raises_exception(self):
        try:
            self.trino_cli.status(self.mock_env)
    ...
    def test_methods_overriden(self):
        assert 'configure' in dir(self.trino_cli)
        assert 'start' in dir(self.trino_cli)
        assert 'stop' in dir(self.trino_cli)
        assert 'status' in dir(self.trino_cli)