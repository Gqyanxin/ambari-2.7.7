...
from package.scripts.trino_coordinator import Coordinator
...
class TestCoordinator(unittest.TestCase):
    ...
    @patch('package.scripts.trino_coordinator.smoketest_trino')
    @patch('package.scripts.trino_coordinator.Coordinator.configure')
    @patch('package.scripts.trino_coordinator.Execute')
    def test_lifecycle_methods_shell_out_to_execute(
            self, execute_mock, unused_configure_mock, unused_smoketest_trino):
        trino_coordinator = Coordinator()

        trino_coordinator.install(self.mock_env)
    ...
    @patch('package.scripts.trino_coordinator.Coordinator.configure')
    @patch('package.scripts.trino_coordinator.Execute')
    def test_install_start_configure_trino(
            self, unused_execute_mock, configure_mock, unused_smoketest_trino):
        trino_coordinator = Coordinator()
    ...
    @patch('package.scripts.trino_coordinator.create_connectors')
    @patch('package.scripts.params.config_properties', new=dummy_config_properties)
    def test_configure_adds_tpch_connector(self, create_connectors_mock):
        ...
        with patch('__builtin__.open', open_mock):
            trino_coordinator.configure(self.mock_env)
    ...
    @patch('package.scripts.trino_coordinator.Coordinator.configure')
    @patch('package.scripts.trino_coordinator.Execute')
    def test_start_smoketests_trino(
            self, execute_mock, unused_configure_mock, smoketest_trino_mock, create_connectors_mock):
        trino_coordinator = Coordinator()

        trino_coordinator.start(self.mock_env)

        assert smoketest_trino_mock.called
    ...
    def test_assert_constant_properties(self, create_connectors_mock):
        ...
        assert 'node.data-dir=/var/lib/trino\n'
    ...
    @patch('package.scripts.params.host_info', new={'trino_coordinator_hosts': ['master']})
    @patch('package.scripts.trino_coordinator.create_connectors')
    @patch('package.scripts.params.config_properties', new=dummy_config_properties)
